...
# Keycloak is versioned independently. compose resolves the image
...
[[ -n "${KEYCLOAK_VERSION:-}" ]] || die "KEYCLOAK_VERSION not set in .env (see ops/.env.example; pin the desired kumbuka-keycloak release)"
...
if wait_containers_healthy 180 && public_healthcheck 60 && org_mapper_check && oidc_issuer_check; then
  log "=== deployed $NEW ==="
  exit 0
fi
