...
public class AdminOverviewResource {
    ...
    public OverviewView get() {
        ...
        // Member summary — pulls from the cached user_account rows. The
        // authoritative list lives in Keycloak; Phase 8 reconciles.
        List<MemberSummary> members = UserAccount.<UserAccount>list(
                "order by email"
            ).stream()
