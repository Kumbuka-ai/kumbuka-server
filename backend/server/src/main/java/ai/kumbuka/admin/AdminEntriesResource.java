 ...
 * shared writes identically. This mirrors the structural-enforcement
 * pattern in {@link AdminScopesResource}. Private scope addressing → 404.
 *
 * <p>The runtime gate is also where the future admin-lock check will
...
public class AdminEntriesResource {
    ...
    @Inject ai.kumbuka.audit.TeamAuditService audit;   // remap governance event
    ...
    public Response create(@PathParam("slug") String slug, CreateEntryRequest req) {
        ...
        // Runtime write-gate: muted members are rejected here, normal
        // members pass. This is also the seam for the lock check.
        writePolicy.assertCanWriteShared(identity.getPrincipal().getName());
        // a locked scope rejects member writes; a team-admin
        ...
        MemoryContentValidator.validate(req.content());   // ≤1500, server-side
        ai.kumbuka.util.MemoryKeyValidator.validate(req.key());   // key format
        ReferenceUrlValidator.validate(req.reference());
        MemoryType t = MemoryType.fromDb(req.type());
        // the console "new entry" path must NOT silently upsert by key
        // createShared rejects an existing key (author-independent)
        ...
        if (req.reference() != null && !req.reference().isBlank()) {
            m.reference = req.reference();   // explicit admin write
        }
    ...
    public EntryView update(@PathParam("slug") String slug,
        ...
        Scope scope = requireSharedSlug(slug);
        writePolicy.assertCanWriteShared(identity.getPrincipal().getName());   // (see create)
        // locked scope rejects member edits; admin overrides.
        boolean isAdmin = callerIsAdmin();
        writePolicy.assertScopeWritable(scope, SourceChannel.CONSOLE, isAdmin);
        MemoryContentValidator.validate(req.content());   // ≤1500, server-side
        ReferenceUrlValidator.validate(req.reference());
        MemoryType t = req.type() == null ? null : MemoryType.fromDb(req.type());
        // the acting admin is stamped as updated_by on the edit.
        // A still-SYSTEM-locked row throws inside update() here — a
        // separate, still-binding axis — so the override audit below never fires
        // for a row the entry-level lock rejected (axis composition).
        Memory m = sharedMemories.update(id, req.content(), t, identity.getPrincipal().getName());
        if (req.reference() != null) {   // null preserves, blank clears
    ...
    public Response delete(@PathParam("slug") String slug, @PathParam("id") UUID id) {
        Scope scope = requireSharedSlug(slug);
        writePolicy.assertCanWriteShared(identity.getPrincipal().getName());   // (see create)
        // locked scope rejects member deletes; admin overrides.
        boolean isAdmin = callerIsAdmin();
        writePolicy.assertScopeWritable(scope, SourceChannel.CONSOLE, isAdmin);
        ...
        // A locked row is rejected by the lock check inside
    ...
    // a team-admin re-homes a shared entry into another shared scope
    // (either direction). Atomic scope_id move (lossless); private excluded
    // structurally; target key-collision reuses the existing-key rejection (KEY_EXISTS, with an
    // optional key override to rename). Recorded as a governance-audit event
    // (scopes/ids only, never content). MCP never reaches this — UI-only.
    @POST
    @Path("/{id}:remap")
    ...
    public EntryView remap(@PathParam("slug") String slug,
        ...
        // remap is admin-only (@RolesAllowed(ROLE_ADMIN)).
    ...
    /**
     * when an admin override write lands on a content-locked
     ...
     * and the SYSTEM entry-lock throws before the call site on a
